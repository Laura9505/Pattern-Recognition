...
def convnet_init(block_n):
    """
    Initialize the weights of a ConvNet with block_n layers, for use with the
...
def convnet_forward(x, block_n, params):
    """
...
def three_layer_convnet_test(block_n):
    ...
    with tf.device(device):
        ...
        logits = convnet_forward(x, block_n, params)
...
with tf.device('/cpu:0'):
    for block_n in [1, 2, 3]:
        three_layer_convnet_test(block_n)
...
def train(model_fn, init_fn, learning_rate, epochs, print_every=100):
    ...
    with tf.device(device):
        ...
        params = init_fn(block_n)  # Initialize the model parameters
        scores = model_fn(x, block_n, params)  # Forward pass of the model
...
def plot_graph(t):
    ...
    if t == 'accuracy':
        plt.plot(np.arange(1, epochs+1), test_accuracies)
        plt.xticks(np.arange(1, epochs+1, 1))
        plt.xlabel('epochs')
...
for block_n in [1, 2, 3]:
    print('number of block = %s'% str(block_n + 1))
